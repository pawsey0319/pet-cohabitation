...
const root=resolve(process.argv[2] ?? "test-results/mobile-keyboard-web");
...
server.listen(Number(process.argv[3] ?? 0),"127.0.0.1",()=>console.log(`UI preview: http://127.0.0.1:${server.address().port}/login`));
