...
if(live){
  const configured=["TEXT_API_BASE_URL","TEXT_API_KEY","TEXT_MODEL"].every(key=>Boolean(Deno.env.get(key)));
  const content=configured?"":await Deno.readTextFile("supabase/functions/.env.local");
