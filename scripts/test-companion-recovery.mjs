...
// Requires the dedicated android-companion-validation stack and COMPANION_SUPABASE_CLI.
...
const workdir = path.resolve(process.env.COMPANION_SUPABASE_WORKDIR ?? "test-results/android-companion-supabase");
...
if (!/^project_id\s*=\s*"android-companion-validation"\s*$/m.test(await readFile(path.join(workdir, "supabase/config.toml"), "utf8"))) throw new Error("Use the dedicated android-companion-validation fixture");
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (system.includes("只提取主人此消息中明确")) {
      ...
      malformedReply = Math.max(0,malformedReply-1);
...
try {
  ...
  spawnSync("docker", ["stop", "supabase_edge_runtime_android-companion-validation"], { windowsHide: true, stdio: "ignore" });
