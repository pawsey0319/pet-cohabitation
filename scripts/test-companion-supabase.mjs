// Full Supabase/Realtime/Edge validation. Only this run's synthetic accounts are created and removed.
...
const explicitlySelectedCloud = endpoint.protocol === "https:" && endpoint.hostname === process.env.COMPANION_TEST_HOST;
if ((!local && !explicitlySelectedCloud) || !["http:", "https:"].includes(endpoint.protocol) || endpoint.username || endpoint.password) {
  throw new Error("Use a loopback URL or explicitly select COMPANION_TEST_HOST for a cloud validation.");
}
...
async function watch(client, ownerId, events) {
  ...
  channel.on("system",{},event=>{if(event.status==="error")console.error("Realtime subscription error:",event.message);});
...
async function invoke(client, name, body) {
  const result = await client.functions.invoke(name, { body: name === "pet-chat" ? {mode:"companion",...body} : body });
  if (result.error) {
...
try {
  ...
  const asset = ok(await service.from("pet_visual_assets").insert({pet_id:pet.id,owner_id:owner.id,storage_path:`fixture/${randomUUID()}.png`,prompt_hash:"synthetic-fixture",is_draft:true}).select("id").single());
  ok(await service.from("pet_expectation_drafts").insert({pet_id:pet.id,owner_id:owner.id,personality_seed_prompt:"Synthetic calm pet personality",visual_seed_prompt:"Synthetic pixel creature",seed_summary:"Synthetic fixture"}));
  ok(await service.rpc("confirm_pet_asset",{target_owner:owner.id,target_asset:asset.id}));
  ...
  if (expectedProvider) {
    const run = await waitUntil(async () => ok(await service.from("model_runs").select("provider,model").eq("owner_id", owner.id).eq("run_kind", "pet_private_reply").eq("status", "succeeded").order("created_at", { ascending: false }).limit(1).maybeSingle()), row => Boolean(row), "model run completion");
  ...
  const claim = ok(await service.rpc("claim_pet_private_request", { request_mode:"companion", target_pet_id: pet.id, request_id: conflictId, owner_content: "继续聊面试" }));
  ...
  const retry = ok(await service.rpc("claim_pet_private_request", { request_mode:"companion", target_pet_id: pet.id, request_id: conflictId, owner_content: "继续聊面试" }));
  ...
  checks++;
  // Android legacy/new API compatibility and strict mode boundaries.
  await assert.rejects(invoke(owner.client,"pet-chat",{...body,content:"different content"}),/private_request_content_changed/);
  await assert.rejects(invoke(owner.client,"pet-chat",{...body,mode:"steward"}),/private_request_mode_changed/);
  const jobsBefore = ok(await service.from("pet_memory_extraction_jobs").select("source_message_id").eq("pet_id",pet.id)).length;
  const stewardBody={mode:"steward",content:"你好",request_id:randomUUID()};
  const stewardReply=await invoke(owner.client,"pet-chat",stewardBody);
  assert.equal(stewardReply.conversation_kind,"steward");
  assert.deepEqual(stewardReply.memory_evidence_ids,[]);
  assert.deepEqual(stewardReply.manual_memory_ids,[]);
  const stewardOwner=ok(await service.from("pet_private_threads").select("*").eq("id",stewardReply.in_reply_to_id).single());
  assert.equal(stewardOwner.reply_status,"succeeded");
  assert.equal((await invoke(second,"pet-chat",stewardBody)).id,stewardReply.id);
  const invalidMemory=await owner.client.rpc("save_pet_personal_memory",{target_pet_id:pet.id,memory_content:"不能把消息管家当来源",source_message_id:stewardOwner.id});
  assert.ok(invalidMemory.error);
  const invalidJob=await service.from("pet_memory_extraction_jobs").insert({source_message_id:stewardOwner.id,pet_id:pet.id,owner_id:owner.id});
  assert.ok(invalidJob.error);
  const oldKey=randomUUID();
  const oldOwner=ok(await service.from("pet_private_threads").insert({pet_id:pet.id,owner_id:owner.id,role:"owner",content:"旧版已发送的问题",request_key:oldKey,reply_status:"succeeded"}).select("id").single());
  const oldReply=ok(await service.from("pet_private_threads").insert({pet_id:pet.id,owner_id:owner.id,role:"pet",content:"旧版已有回答",in_reply_to_id:oldOwner.id}).select("id").single());
  // Direct call deliberately omits mode like the installed old client.
  const adopted=ok(await owner.client.functions.invoke("pet-chat",{body:{content:"旧版已发送的问题",request_id:oldKey}}));
  assert.equal(adopted.id,oldReply.id);
  const oldNew=ok(await owner.client.functions.invoke("pet-chat",{body:{content:"你好",request_id:randomUUID()}}));
  assert.equal(oldNew.conversation_kind,"legacy");
  assert.equal(ok(await service.from("pet_memory_extraction_jobs").select("source_message_id").eq("pet_id",pet.id)).length,jobsBefore);
  const guarded=ok(await service.rpc("claim_pet_private_request",{request_mode:"steward",target_pet_id:pet.id,request_id:randomUUID(),owner_content:"隔离校验"}));
  assert.ok(guarded.message_id);
  checks++;
  console.log(`PASS: ${checks} ${local ? "local" : "cloud"} Supabase/Realtime/Edge scenario groups; ${recoveredErrors.length} recovered provider errors.`);
} finally {
