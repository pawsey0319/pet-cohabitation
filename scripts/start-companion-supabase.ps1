...
$companionWorkdir = Join-Path $companionRoot 'test-results/companion-supabase'
...
if ((Test-Path -LiteralPath $companionConfigPath) -and
    (Get-Content -LiteralPath $companionConfigPath -Raw) -notmatch '(?m)^project_id\s*=\s*"companion-validation"\s*$') {
...
@'
project_id = "companion-validation"
[api]
enabled = true
port = 55321
...
Write-Output 'Isolated companion-validation stack is ready on port 55321; test variables loaded into this terminal.'
