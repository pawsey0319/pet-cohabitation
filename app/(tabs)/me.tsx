...
export default function MeScreen() {
  ...
  return (
    <ScrollView style={styles.page} contentContainerStyle={[styles.content, { paddingTop: insets.top + spacing.lg }]}>
      ...
      <Surface style={styles.section}><Text style={styles.sectionTitle}>我的数据</Text><Text style={styles.copy}>可以导出自己的账号、本人消息、异宠私聊、个人记忆、成长札记与形态谱系，不包含其他成员的私聊内容。</Text><AppButton label={dataBusy ? "处理中…" : "导出我的数据"} variant="quiet" disabled={dataBusy} onPress={() => void exportData()} /><AppButton label="注销账号并删除数据" variant="danger" disabled={dataBusy} onPress={() => { setPassword(""); setDeleting(true); }} />{dataMessage ? <Text style={styles.dataMessage}>{dataMessage}</Text> : null}</Surface>
