...
function MediaContent({ message, signedUrl }: Readonly<{ message: ChatMessage; signedUrl: string | null }>) {
  if (!message.mediaPath) return null;
  if (!signedUrl) return <ActivityIndicator color={colors.mint} style={{ margin: 15 }} />;
  if (message.kind === "image") return <Image source={{ uri: signedUrl }} resizeMode="cover" style={styles.messageImage} />;
  ...
  return (
    <View style={[styles.messageWrap, mine && styles.messageWrapMine]}>
      ...
      <View style={[styles.messageColumn, mine && styles.messageColumnMine]}>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={`消息：${message.text ?? message.kind}`} onPress={onSelect} onLongPress={onSelect} style={[styles.bubble, mine ? styles.bubbleMine : isAgent ? styles.bubbleAgent : styles.bubbleOther, message.kind === "image" && styles.mediaBubble]}>
          ...
          <MediaContent message={message} signedUrl={signedUrl} />
...
const styles = StyleSheet.create({
  page: { flex: 1, backgroundColor: colors.canvas }, center: { flex: 1, justifyContent: "center" }, header: { minHeight: 68, flexDirection: "row", alignItems: "center", paddingHorizontal: spacing.sm, paddingBottom: 7, borderBottomWidth: 1, borderBottomColor: colors.line, backgroundColor: colors.canvasRaised },
  headerButton: { width: 46, height: 46, alignItems: "center", justifyContent: "center" }, headerButtonText: { color: colors.text, fontSize: 38, fontWeight: "300" }, more: { color: colors.text, fontSize: 18, letterSpacing: 2 }, headerTitleWrap: { flex: 1, alignItems: "center" }, headerTitle: { color: colors.text, fontWeight: "900", fontSize: 17 }, headerStatus: { color: colors.mint, fontSize: 10, marginTop: 2 },
  ...
  messageWrap: { flexDirection: "row", gap: 9, alignItems: "flex-start", maxWidth: "88%" }, messageWrapMine: { alignSelf: "flex-end", flexDirection: "row-reverse" }, senderAvatar: { width: 36, height: 36, borderRadius: 13, backgroundColor: colors.surfaceSoft, alignItems: "center", justifyContent: "center" }, senderAvatarAgent: { backgroundColor: colors.mintDeep, borderWidth: 1, borderColor: colors.mint }, senderAvatarText: { color: colors.text, fontWeight: "900" },
  messageColumn: { alignItems: "flex-start", maxWidth: "88%" }, messageColumnMine: { alignItems: "flex-end" }, senderName: { color: colors.textMuted, fontSize: 11, marginLeft: 4, marginBottom: 4 }, agentName: { color: colors.mint },
  bubble: { borderRadius: 16, paddingHorizontal: 13, paddingVertical: 10, overflow: "hidden" }, bubbleOther: { backgroundColor: colors.surface }, bubbleMine: { backgroundColor: colors.coralSoft, borderTopRightRadius: 5 }, bubbleAgent: { backgroundColor: colors.mintDeep, borderWidth: 1, borderColor: "rgba(125,226,196,.35)" }, mediaBubble: { padding: 4 },
