...
- 用户已有公司内部 API，拟用于 Grok 和后续替代旧网关。随后用户在仓库外本地配置文件中填写凭据，2026-09-24 以 Bearer 鉴权读取模型目录返回 HTTP 200、27 个模型条目，包含 3 个 Grok ID。仅验证本机目录访问，文本/流式兼容、实际模型生成及云端可达性尚未验证，结论与边界见 [ENVIRONMENT.md](ENVIRONMENT.md)。此次未安装 CPA、未调用 Grok 推理、未切换 Secrets/隧道，也未停止旧机服务。
