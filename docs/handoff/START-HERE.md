...
September 22 完成：应用 87 套件 626 项全量通过，之后新增隐藏页在途回复用例所在 21 项定向通过；类型检查通过。四模块 20 轮浏览器检查、长对话滚动保留通过；真实云端两秒延迟作用于 28 个读取时，80 次暖切换 P95 37 ms。这个数不是 Android 手机性能。

September 24 接续复验：从远端干净 clone 安装依赖，**87 套件 627 项全量、类型检查、11 项桌面逻辑检查、网页导出及交接证据/原生基线校验均通过**。本次只修正了测试目录忽略规则的可移植性，没有再次部署线上。提交、备份范围和复验详情见 [REPOSITORY-AUDIT.md](REPOSITORY-AUDIT.md)。
