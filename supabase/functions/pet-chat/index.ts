...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { error: ownerInsertError } = await client.from("pet_private_threads").insert({ pet_id: pet.id, owner_id: user.id, role: "owner", content: input.content });
    ...
    const reply = await adapter.generatePetReply({
      ...
      messages: [...(thread ?? [])].reverse().map((message) => ({ actor: message.role === "owner" ? "主人" : pet.name, content: message.content })),
      currentMessage: input.content, ownerPolicy: "pet_only",
    });
    const { data: inserted, error: insertError } = await client.from("pet_private_threads").insert({ pet_id: pet.id, owner_id: user.id, role: "pet", content: reply.content, model_run_id: runId }).select("id,content,created_at").single();
    ...
    if (pet.status === "confirmed") await client.from("pet_experiences").insert({ pet_id: pet.id, owner_id: user.id, category: "shared", summary: `主人和${pet.name}聊了一段只属于彼此的话：${input.content.slice(0, 180)}` });
    ...
    return json(request, { id: inserted.id, content: inserted.content, created_at: inserted.created_at });
  } catch (reason) {
