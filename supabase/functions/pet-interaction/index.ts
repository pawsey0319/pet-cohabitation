...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const experience = await client.from("pet_experiences").insert({ pet_id: pet.id, owner_id: pet.owner_id, space_id: input.space_id, category: input.action === "play" ? "social" : "care", summary: content }).select("id").single();
