...
export async function localDataKeys(ownerId: string): Promise<string[]> {
  ...
  const keys = (await AsyncStorage.getAllKeys()).filter((key) => key.startsWith("pet-cohabitation") || key.startsWith("pet-chat-background-v1:") || key.startsWith("pet-chat-messages-v1:") || key.startsWith("pet-work-v1:") || key.startsWith("pet-push-") || key.startsWith("pet-private-send-queue-v1:"));
